  ...
  return (
    <header className={overlay ? "absolute inset-x-0 top-0 z-50 text-paper" : "sticky top-0 z-50 border-b border-cocoa/10 bg-ivory/90 backdrop-blur-xl"}>
      ...
      <div className="shell flex min-h-20 items-center justify-between gap-4">
        <Link href="/" className="logo-link group flex items-center gap-3" aria-label="Splot Kamieni, strona główna">
          <span className="logo-mark relative size-12 overflow-hidden rounded-full border border-paper/50 bg-paper shadow-lg"><Image src="/jewelry/splot-kamieni-logo.webp" alt="" fill className="object-cover" sizes="48px" /></span>
          <span className="hidden sm:block"><span className="block font-editorial text-xl font-semibold leading-none">Splot Kamieni</span><span className={`mt-1 block text-[0.58rem] font-bold uppercase tracking-[0.18em] ${overlay ? "text-paper/60" : "text-cocoa/55"}`}>Wersja demonstracyjna</span></span>
        </Link>
